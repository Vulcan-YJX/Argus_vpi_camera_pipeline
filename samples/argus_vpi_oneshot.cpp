...
int main(int argc, char ** argv)
{
  ...
  Argus::CameraDevice * device =
    ArgusSamples::ArgusHelpers::getCameraDevice(cameraProvider.get(), options.cameraDeviceIndex());
  ...
  printf(
    "Capturing from device %d using sensor mode %d (%dx%d)\n", options.cameraDeviceIndex(),
    options.sensorModeIndex(), iSensorMode->getResolution().width(),
  ...
  if (m_dmabuf == -1) {
    m_dmabuf = iNativeBuffer->createNvBuffer(
      iSensorMode->getResolution(), NVBUF_COLOR_FORMAT_BGRA, NVBUF_LAYOUT_PITCH);
    if (m_dmabuf == -1) {
      ...
      if (nvbuf_surf->surfaceList[0].colorFormat == NVBUF_COLOR_FORMAT_BGRA) {
        ...
        CHECK_STATUS(
          vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CUDA, srcImage, dstImage, NULL));

        CHECK_STATUS(vpiStreamSync(stream));

        VPIImageData outData;
        CHECK_STATUS(
          vpiImageLockData(dstImage, VPI_LOCK_READ, VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR, &outData));
        cv::Mat cvImage;

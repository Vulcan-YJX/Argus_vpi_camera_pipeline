...
const cv::Mat camera_matrix =
  (cv::Mat_<double>(3, 3) << 964.431946, 0.000000, 951.792114, 0.000000, 964.806641, 582.802063,
   0.000000, 0.000000, 1.000000);

const cv::Mat distortion_coefficients =
  (cv::Mat_<double>(1, 8) << 0.854475, 0.144149, 0.001531, 1.227936, 0.364355, 0.020056, -0.000219,
   0.000041);
...
int main()
{
  ...
  for (int i = 0; i < num_cameras; ++i) {
    pipelines[i] =
      "nvarguscamerasrc sensor_id=" + std::to_string(i) +
  ...
  cv::initUndistortRectifyMap(
    camera_matrix, distortion_coefficients, cv::Mat(), camera_matrix, image_size, CV_16SC2, map1,
    map2);
